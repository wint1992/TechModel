	...
	public GraphQLSchemaModel(HibernateUtil _hibernate){
		...
		queryType = newObject().name("QueryType")
				.field(newFieldDefinition().name("event").type(list(EventType))
						.argument(newArgument().name("id").type(Scalars.GraphQLInt)).dataFetcher(dataFetcher.eventFetcher))
				.field(newFieldDefinition().name("profile").type(list(ProfileType))
						.argument(newArgument().name("id").type(Scalars.GraphQLInt)).dataFetcher(dataFetcher.profileFetcher))
				.field(newFieldDefinition().name("city").type(list(CityType))
						.argument(newArgument().name("id").type(Scalars.GraphQLInt)).dataFetcher(dataFetcher.cityFetcher))
				.field(newFieldDefinition().name("musicStyle").type(list(EventMusicStyleType))
						.argument(newArgument().name("id").type(Scalars.GraphQLInt)).dataFetcher(dataFetcher.musicStyleFetcher))
				.field(newFieldDefinition().name("eventAddress").type(list(EventAddressType))
						.argument(newArgument().name("id").type(Scalars.GraphQLInt)).dataFetcher(dataFetcher.eventAddressFetcher)).build();
